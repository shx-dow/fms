...
const POST = async ({ request, locals }) => {
  if (!locals.user || !["HOD", "ADMIN"].includes(locals.user.role)) return json({ ok: false, error: "Only HOD or Admin may reopen a report." }, { status: 403 });
  const { reportId, reason, allowedUntil } = await request.json();
  if (!reportId || !reason || !allowedUntil) return json({ ok: false, error: "Report, reason, and expiry are required." }, { status: 400 });
