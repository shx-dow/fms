...
export const load = ({ locals }: Parameters<PageServerLoad>[0]) => {
  ...
  const report = sqlite.prepare('SELECT r.*, u.name AS faculty_name, u.email AS faculty_email, p.label AS period_label FROM reports r JOIN users u ON u.id = r.faculty_id JOIN reporting_periods p ON p.id = r.period_id WHERE r.faculty_id = ? ORDER BY r.updated_at DESC LIMIT 1').get(locals.user.id) as Record<string, unknown> | undefined;
