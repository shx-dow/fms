...
{:else}
  ...
  {#if showPasswordModal}
    <Modal title="Change password" onclose={() => (showPasswordModal = false)}>
      ...
      <div class="modal-acts">
        <button class="btn" onclick={() => (showPasswordModal = false)}>Cancel</button>
        <button class="btn btn-primary" disabled={pwSaving} onclick={changePassword}>{pwSaving ? 'Saving…' : 'Update password'}</button>
      </div>
    </Modal>
  {/if}
  {#if showProfileModal}
    <Modal title="Recurring report template" size="lg" onclose={() => (showProfileModal = false)}>
      <p class="pw-sub">Save the details that repeat each week. They will be pre-filled when you start a new report.</p>
      <div class="profile-tip"><span>Only add information that stays the same. You can edit the weekly details in each report.</span></div>
        ...
        <div class="modal-acts"><button class="btn" onclick={() => (showProfileModal = false)}>Cancel</button><button class="btn btn-primary" disabled={profileSaving} onclick={saveProfile}>{profileSaving ? 'Saving…' : 'Save profile'}</button></div>
    </Modal>
  {/if}
  <Toast />
{/if}
...
<style>
  ...
  @media (max-width: 720px) { .profile-labels { display: none; } .profile-row, .profile-row.subject-row, .profile-row.two-row { grid-template-columns: 1fr 1fr 58px; } .profile-row.subject-row input:first-child, .profile-row.subject-row input:nth-child(2) { grid-column: span 1; } }
