...
export const GET: RequestHandler = ({ locals }) => {
  ...
  return json({ records: sqlite.prepare('SELECT * FROM institutional_duties WHERE report_id = ? ORDER BY rowid').all(reportId) });
};
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  for (const r of records) {
    sqlite.prepare('INSERT INTO institutional_duties (id, report_id, name, role, activity, reach, outcome) VALUES (?, ?, ?, ?, ?, ?, ?)').run(randomUUID(), reportId, r.name, r.role, r.activity ?? null, r.reach ?? null, r.outcome ?? null);
  }
  return json({ ok: true });
};
