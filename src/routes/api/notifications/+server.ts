...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  if (payload.eventId) {
    sqlite.prepare('INSERT OR REPLACE INTO notification_reads (user_id, event_id, read_at) VALUES (?, ?, ?)').run(locals.user.id, String(payload.eventId), now);
  } else {
    const events = sqlite.prepare('SELECT id FROM audit_events').all() as { id: string }[];
    const insert = sqlite.prepare('INSERT OR REPLACE INTO notification_reads (user_id, event_id, read_at) VALUES (?, ?, ?)');
