...
import { outreachRecordSchema, reportRecordsSchema } from '$lib/server/validation';
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const parsedRecords = z.array(outreachRecordSchema).safeParse(parsed.data.records);
  if (!parsedRecords.success)
    return json({ ok: false, error: parsedRecords.error.issues.map(i => i.message).join('; ') }, { status: 400 });
  replaceOutreach(reportId, parsedRecords.data);
  if (parsed.data.empty !== undefined) setSectionEmpty(reportId, 'outreach', parsed.data.empty);
  return json({ ok: true });
};
