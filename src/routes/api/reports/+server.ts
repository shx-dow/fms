...
export const GET: RequestHandler = ({ locals }) => {
  ...
  const userId = locals.user.id; const period = currentPeriod(); if (!period) return json({ error: 'No open reporting period.' }, { status: 409 });
  let report = sqlite.prepare('SELECT * FROM reports WHERE faculty_id = ? AND period_id = ? LIMIT 1').get(userId, period.id) as Record<string, unknown> | undefined;
  if (!report) {
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  if (!locals.user) return json({ error: 'Unauthorized' }, { status: 401 });
  const userId = locals.user.id; const period = currentPeriod(); if (!period) return json({ ok: false, error: 'No open reporting period.' }, { status: 409 });
  ...
  const existing = sqlite.prepare('SELECT status, reopened_until FROM reports WHERE id = ? AND faculty_id = ?').get(reportId, userId) as { status: string; reopened_until?: string | null } | undefined;
  ...
  const aliases: Record<string, string> = { course_code: 'courseCode', course_name: 'courseName', program_level: 'programLevel', class_type: 'classType', missed_action: 'missedAction', syllabus_completion: 'syllabusCompletion', venue_or_agency: 'venueOrAgency', indexing_or_quality: 'indexingOrQuality' };
  const replace = (table: string, rows: Record<string, unknown>[], columns: string[]) => { sqlite.prepare(`DELETE FROM ${table} WHERE report_id = ?`).run(reportId); for (const row of rows ?? []) { const values = columns.map((column) => { const value = row[column] ?? row[aliases[column]]; return ['scheduled', 'conducted', 'missed'].includes(column) ? Number(value ?? 0) : value ?? null; }); sqlite.prepare(`INSERT INTO ${table} (id, report_id, ${columns.join(', ')}) VALUES (?, ?, ${columns.map(() => '?').join(', ')})`).run(randomUUID(), reportId, ...values); } };
