...
export const GET: RequestHandler = ({ locals, url }) => {
  ...
  return json({ records: listResearch(reportId) });
};
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const parsedRecords = z.array(researchRecordSchema).safeParse(parsed.data.records);
  if (!parsedRecords.success)
    return json({ ok: false, error: parsedRecords.error.issues.map(i => i.message).join('; ') }, { status: 400 });
  replaceResearch(reportId, parsedRecords.data);
  if (parsed.data.empty !== undefined) setSectionEmpty(reportId, 'research', parsed.data.empty);
  return json({ ok: true });
};
