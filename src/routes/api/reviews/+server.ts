...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  if (!reportId || !['APPROVED', 'CHANGES_REQUIRED'].includes(decision))
    return json({ ok: false, error: 'Invalid review decision' }, { status: 400 });
  ...
  const allowed = sqlite
    ...
    .get(...(locals.user.role === 'HOD' ? [reportId, 'cse'] : [reportId]));
