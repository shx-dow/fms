...
export const GET: RequestHandler = ({ locals }) => {
  ...
  const rows = sqlite.prepare(`SELECT r.id, r.status, r.updated_at, r.completion, u.name AS faculty_name, p.label AS period_label FROM reports r JOIN users u ON u.id = r.faculty_id JOIN reporting_periods p ON p.id = r.period_id WHERE r.status IN ('SUBMITTED','CHANGES_REQUIRED','APPROVED')${scope} ORDER BY r.updated_at DESC`).all(...(locals.user.role === 'HOD' ? ['cse'] : []));
