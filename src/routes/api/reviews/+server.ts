...
export const GET: RequestHandler = ({ locals }) => {
  ...
  if (fmt === 'csv') {
    ...
    const body = (rows as any[]).map((r) => `"${r.faculty_name}","${r.faculty_email}","${r.period_label}","${r.status}",${r.completion},${r.submitted_at ? new Date(r.submitted_at).toISOString() : ''},${new Date(r.updated_at).toISOString()}`).join('\n');
