<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (d.teaching?.length)
      teaching = d.teaching.map((item: any) => ({
        courseCode: item.course_code,
        courseName: item.course_name,
        programLevel: item.program_level,
        classType: item.class_type,
        scheduled: item.scheduled,
        conducted: item.conducted,
        missed: item.missed,
        missedAction: item.missed_action ?? '',
        syllabusCompletion: item.syllabus_completion ?? 0,
      }));
    weeklySummary = d.report.summary ?? '';
    researchRecords = (d.research ?? []).map((r: any) => ({
      category: r.category,
      title: r.title,
      venueOrAgency: r.venue_or_agency ?? '',
      indexingOrQuality: r.indexing_or_quality ?? '',
      role: r.role ?? '',
      status: r.status ?? '',
    }));
    dutiesRecords = (d.duties ?? []).map((r: any) => ({
      name: r.name,
      role: r.role,
      activity: r.activity ?? '',
      reach: r.reach ?? '',
    ...
    outreachRecords = (d.outreach ?? []).map((r: any) => ({
      activity: r.activity,
      audience: r.audience ?? '',
      outcome: r.outcome ?? '',
      date: r.date ?? '',
  ...
  async function loadAttachments() {
    try {
      const r = await fetch(`/api/attachments?reportId=${reportId}`);
  ...
  async function uploadEvidence(event: Event) {
    ...
    fd.set('file', file);
    const response = await fetch('/api/attachments', { method: 'POST', body: fd });
    attachmentMessage = response.ok ? `${file.name} uploaded` : 'Upload failed';
    input.value = '';
  ...
  async function saveDraft(_event?: unknown, status = 'DRAFT') {
    ...
    const response = await fetch('/api/reports', {
      ...
      body: JSON.stringify({ reportId, teaching, summary: weeklySummary, completion, status }),
    });
  ...
  async function saveActivity(kind: 'research' | 'duties' | 'outreach') {
    ...
    const response = await fetch(`/api/${kind}`, {
      ...
      body: JSON.stringify({
        reportId,
        records: kind === 'research' ? researchRecords : kind === 'duties' ? dutiesRecords : outreachRecords,
      }),
    });
  ...
  function removeTeaching(index: number) {
    teaching = teaching.filter((_, i) => i !== index);
    saveDraft();
  }
  async function handleSubmit() {
    confirmSubmit = false;
    const ok = await saveDraft(undefined, 'SUBMITTED');
    if (ok) {
      submitted = true;
...
<main class="shell app-shell editor-shell" onkeydown={handleKeydown}>
  <header class="topbar">
    ...
    <div class="topbar-user">
      {data.user?.name ?? 'User'} <span class="avatar">{(data.user?.name ?? 'U').slice(0, 1)}</span>
    </div>
  </header>
      ...
      <p>
        {isReadonly
          ...
          : reportStatus === 'CHANGES_REQUIRED'
            ? 'Your HOD has requested changes. Review their feedback and resubmit.'
            : "Record this week's teaching activity. Research, duties and appraisal are managed separately."}
      </p>
    </div>
  ...
  {#if submitted}
    <div class="success">
      <strong>Weekly report submitted for review.</strong><span>Your HOD can now review {periodLabel}.</span><a
  ...
  {:else if isReadonly}
    <div class="locked-banner" role="status">
      ...
      <span
        >{reportStatus === 'SUBMITTED'
          ? 'Your HOD is reviewing this report. You will be notified when a decision is made.'
          : 'This weekly report has been approved. No further action is needed.'}</span
      >
    ...
    <details class="review-history-details">
      ...
      {#each reviews as rv}
        <div class="review-history-entry">
          <span class="rh-status {rv.decision === 'APPROVED' ? 'rh-approved' : 'rh-changes'}"
  ...
  <div class="editor-layout">
    <nav class="section-nav" aria-label="Report sections">
      ...
      <div class="scope-note">
        Research, institutional duties, outreach and appraisal are not part of the weekly submission.
      </div>
    </nav>
          ...
          {#each teaching as item, index}
            <article class="record">
              ...
              <div class="field-grid">
                <label
                  >Course code<input bind:value={item.courseCode} oninput={saveDraft} placeholder="CSE-301" /></label
                >
                <label
                  >Course name<input
                    bind:value={item.courseName}
                    oninput={saveDraft}
                ...
                <label
                  >Program / level<input
                    bind:value={item.programLevel}
                    oninput={saveDraft}
                    placeholder="B.Tech · III Year"
                  /></label
                >
                <label
                  >Class type<select bind:value={item.classType} onchange={saveDraft}
              ...
              <div class="number-grid">
                <label
                  >Total scheduled<input
                    type="number"
                    min="0"
                    bind:value={item.scheduled}
                    oninput={saveDraft}
                    placeholder="0"
                  /></label
                >
                <label
                  >Classes taken<input
                    type="number"
                    min="0"
                    bind:value={item.conducted}
                    oninput={saveDraft}
                    placeholder="0"
                  /></label
                >
                <label
                  >Classes missed<input
                    type="number"
                    min="0"
                    bind:value={item.missed}
                    oninput={saveDraft}
                    placeholder="0"
                  /></label
                >
                <label
                  >Syllabus completion %<input
                    type="number"
                    min="0"
                    max="100"
                    bind:value={item.syllabusCompletion}
                    oninput={saveDraft}
                    placeholder="0"
                  /></label
                >
              ...
              <label
                >Action taken for missed classes<input
                  bind:value={item.missedAction}
                  oninput={saveDraft}
                  placeholder="e.g. Makeup class scheduled"
                /></label
              >
            </article>
          {/each}
      ...
      {:else if activeSection === 'summary'}
        <div class="section-intro">
          <div>
            ...
            <p>
              Provide a brief narrative of this week's teaching activities, notable events, or student interactions.
            </p>
          </div>
        </div>
        <textarea
          bind:value={weeklySummary}
          oninput={saveDraft}
          rows="6"
          placeholder="Summarise the week's teaching, student engagement, and any notable academic events…"
          style="width:100%;border:1px solid #cbd7de;border-radius:6px;padding:11px;font:inherit;background:#fbfcfc;resize:vertical"
        ></textarea>
      {:else if activeSection === 'additional'}
        <div class="additional-tabs">
          <button
            class:active={activeExtraSection === 'attachments'}
            onclick={() => (activeExtraSection = 'attachments')}><span>📎</span>Files ({attachments.length})</button
          ><button class:active={activeExtraSection === 'research'} onclick={() => (activeExtraSection = 'research')}
            ><span>📄</span>Research</button
          ><button class:active={activeExtraSection === 'duties'} onclick={() => (activeExtraSection = 'duties')}
            ><span>🏛</span>Duties</button
          ><button class:active={activeExtraSection === 'outreach'} onclick={() => (activeExtraSection = 'outreach')}
            ><span>🤝</span>Outreach</button
          >
        </div>
        <div class="extra-panel">
          {#if activeExtraSection === 'attachments'}
            <div class="section-intro">
              <div>
                ...
                <p>Upload PDF, PNG or JPEG files (max 10 MB each) to support your report.</p>
              </div>
            </div>
            <div class="attachment-grid">
              {#each attachments as a}<div class="attachment-card">
                  <div class="attach-icon">{a.mime_type.includes('pdf') ? '📄' : '🖼'}</div>
                  <div class="attach-info">
                    <strong>{fileName(a.filename)}</strong><small
                      >{formatSize(a.size)} · {new Date(a.created_at).toLocaleDateString()}</small
                    >
                  </div>
                  <div class="attach-actions">
                    <a class="dl-link" href="/api/attachments/{a.id}" download>Download</a>{#if canEdit}<button
                        class="remove-attach"
                        onclick={() => deleteAttachment(a.id)}>Delete</button
                      >{/if}
                  </div>
                </div>{:else}<div class="empty-section">No files uploaded yet.</div>{/each}
            </div>
            {#if canEdit}<div class="upload-area">
          ...
          {:else if activeExtraSection === 'research'}
            <div class="section-intro">
              <div>
                <div class="eyebrow">Ongoing record</div>
                <h2>Research & publications</h2>
                <p>Maintain work that spans multiple weeks without repeating it in every report.</p>
              </div>
            </div>
            {#each researchRecords as item}<div class="ongoing-record">
                <div class="field-grid">
                  <label
                    >Category<select bind:value={item.category}
                      ><option>Journal Paper</option><option>Patent</option><option>Research Grant</option><option
                        >Conference / FDP</option
                      ></select
                    ></label
                  ><label>Title<input bind:value={item.title} placeholder="Title of paper, patent or project" /></label
                  ><label>Venue / agency<input bind:value={item.venueOrAgency} /></label><label
                    >Status<input bind:value={item.status} placeholder="In progress, submitted, published" /></label
                  >
                </div>
              </div>{/each}
            <div class="ongoing-actions">
              ...
              ><button class="quiet" onclick={() => saveActivity('research')}
                >{additionalSaving ? 'Saving…' : 'Save research'}</button
              >
            </div>
          {:else if activeExtraSection === 'duties'}
            <div class="section-intro">
              <div>
                ...
                <p>Keep committee, club and departmental responsibilities current.</p>
              </div>
            </div>
            {#each dutiesRecords as item}<div class="ongoing-record">
                <div class="field-grid">
                  <label>Committee / body<input bind:value={item.name} /></label><label
                    >Role<input bind:value={item.role} /></label
                  ><label>Activity<input bind:value={item.activity} /></label><label
                    >Outcome<input bind:value={item.outcome} /></label
                  >
                </div>
              </div>{/each}
            <div class="ongoing-actions">
              <button
                class="add-record"
                onclick={() =>
                  (dutiesRecords = [...dutiesRecords, { name: '', role: '', activity: '', reach: '', outcome: '' }])}
                >+ Add duty</button
              ><button class="quiet" onclick={() => saveActivity('duties')}
                >{additionalSaving ? 'Saving…' : 'Save duties'}</button
              >
            </div>
          {:else if activeExtraSection === 'outreach'}
            <div class="section-intro">
              <div>
                ...
                <p>Log events and engagement activities when they happen.</p>
              </div>
            </div>
            {#each outreachRecords as item}<div class="ongoing-record">
                <div class="field-grid">
                  <label>Activity<input bind:value={item.activity} /></label><label
                    >Audience<input bind:value={item.audience} /></label
                  ><label>Date<input type="date" bind:value={item.date} /></label><label
                    >Outcome<input bind:value={item.outcome} /></label
                  >
                </div>
              </div>{/each}
            <div class="ongoing-actions">
              <button
                class="add-record"
                onclick={() =>
                  (outreachRecords = [...outreachRecords, { activity: '', audience: '', outcome: '', date: '' }])}
                >+ Add outreach</button
              ><button class="quiet" onclick={() => saveActivity('outreach')}
                >{additionalSaving ? 'Saving…' : 'Save outreach'}</button
              >
            </div>
          {/if}
        </div>
  ...
  {#if !isReadonly}
    <section class="evidence-panel">
      <div>
        ...
        <p>
          {attachments.length} file{attachments.length === 1 ? '' : 's'} attached{attachments.length
            ? ` · ${attachments.map((a) => fileName(a.filename)).join(', ')}`
            : ''}.
        </p>
      </div>
      <div class="panel-mini-actions">
        <button
          ...
          onclick={() => {
            ...
            activeExtraSection = 'attachments';
          }}>Manage files</button
        >
      </div>
    </section>
...
<style>
  ...
  .review-history-entry:last-child {
    border-bottom: 0;
  }
  .rh-status {
    flex: none;
